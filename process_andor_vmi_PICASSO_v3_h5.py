...
# -*- coding: utf-8 -*-
"""
VMI Photoelectron Analysis - PICASSO V3 (HDF5 Input)
=====================================================
Adapted from v2 for HDF5 data format.

Key differences from v2:
- Reads frames from HDF5 files (raw/data, raw/meta) instead of TIFF
- Each HDF5 file contains 126 frames — worker processes one file at a time
- GMD extracted from raw/meta column instead of TIFF tags
- `tifffile` replaced with `h5py`

Data model:
  HDF5 file structure:
    raw/data  -> (N_frames, height, width)  uint16
    raw/meta  -> (N_frames, N_columns)       uint64  (GMD in configurable column)

Author: TAO Jianfei (jakiesumrain@163.com)
Date: 2026-06-27 (v3: HDF5 adaptation)
"""
...
META_COLUMN_GMD = vmi_cfg.get("meta_column_gmd", 1)  # column index for GMD in raw/meta (DEPRECATED — see GMD_MAP_PATH)

# Pre-computed frame-to-GMD map file (overrides raw/meta for GMD extraction)
# Generated by build_gmd_map.py using timestamp matching against current-time-both.txt
# Columns: frame_idx, h5_file, frame_in_file, gmd_idx, gmd_val1, gmd_val2, time_diff_ms
# Path read from config.toml (gmd_map_path). Empty string = fall back to raw/meta column.
...
# Dark data can come from a separate HDF5 directory (same structure: raw/data)
...
def load_flat_field():
    ...
    # Flat field is a single image; keep tifffile import local
    try:
        import tifffile
    except ImportError:
...
# ==============================================================================
# The map file is generated by build_gmd_map.py:
#   frame_idx → GMD entry via timestamp matching (col3 + col4/1e9 vs current-time-both.txt col2)
# This is necessary because the GMD and image data are recorded on different
# machines with unsynchronized clocks — only the timestamps on the same machine
# (machine-B in current-time-both.txt, column 2) can be used for correlation.
...
def _run_picasso_pipeline(corrected, box_size, min_net_gradient):
    """Run identify + MLE fit on a single-frame movie. Returns locs DataFrame
    or None if no spots found. Accepts float64 (preferred) or uint16."""
    movie = corrected[np.newaxis, :, :]  # (1, H, W)
...
def picasso_to_trackpy_style(locs, frame_num):
    ...
    df['signal'] = locs['photons'] / (np.pi * size_safe**2)
    df['frame'] = frame_num
...
def _process_single_frame(frame_data, frame_num, dark_frame, flat_field_normalized,
                          box_size, min_net_gradient):
    """Apply corrections + Picasso pipeline to one frame. Returns features or None."""
    try:
        if dark_frame is not None:
            corrected = frame_data.astype(np.float64) - dark_frame
        else:
            corrected = frame_data.astype(np.float64)
        ...
        locs = _run_picasso_pipeline(corrected, box_size, min_net_gradient)

        if locs is not None and len(locs) > 0:
            features = picasso_to_trackpy_style(locs, frame_num=frame_num)
            if features is not None and len(features) > 0:
                return features
        return None

    except Exception as e:
        print(f"[Warning] Frame {frame_num} failed: {e}")
        return None

def process_h5_file_worker(args):
    """Worker function for parallel Picasso processing.

    Processes all frames from one HDF5 file in a single call.
    Opens the file once instead of once per frame.
    ...
    try:
        with h5py.File(h5_path, 'r') as hf:
            data = hf[H5_DATASET][:]  # (N_frames, H, W)
    except Exception as e:
    ...
    for i, frame in enumerate(data):
        frame_num = base_frame_idx + i
        result = _process_single_frame(frame, frame_num, dark_frame,
                                        flat_field_normalized, box_size,
                                        min_net_gradient)
...
def run_batch_processing_picasso(h5_files, frame_info, dark_frame, flat_field_normalized):
    """Run Picasso localization on all frames, processing one HDF5 file per task.

    Streams per-file results directly to an HDF5 checkpoint to avoid holding
    all per-file DataFrames in memory simultaneously.
    ...
    # Track base frame index for continuous global frame numbering
    h5_batch_args = []
    idx = 0
    for h5_path in h5_files:
        with h5py.File(h5_path, 'r') as hf:
    ...
    print(f"[Batch] Processing {n_files} HDF5 files ({len(frame_info)} frames) "
          f"with Picasso (MLE + Net Gradient)...")
    ...
    with pd.HDFStore(checkpoint_path, mode="w", complevel=1) as store:
        if ENABLE_PARALLEL:
            with Pool(n_cores) as pool:
                for result in tqdm(
                    pool.imap(process_h5_file_worker, h5_batch_args),
                    total=n_files,
            ...
            for args in tqdm(h5_batch_args):
                result = process_h5_file_worker(args)
                if result is not None:
                    store.append("localizations", result, index=False)
                    n_total += len(result)
                    files_with_detections += 1
...
def plot_final_summary(vmi_2d, vmi_3d, n_events, n_frames):
    ...
    summary_text = f"""PICASSO ANALYSIS COMPLETE (memory-bounded)
...
def run_tuning_check(frame_info, dark_frame, flat_field_normalized):
    ...
    # Pick the frame at TUNING_FRAME_INDEX (clamped to available range)
...
def main():
    print("=" * 80)
    print("VMI PHOTOELECTRON ANALYSIS - PICASSO V3 (HDF5 Input)")
    print("=" * 80)
